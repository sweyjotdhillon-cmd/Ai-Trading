...
- **Pointwise Inequalities Matrix (`evaluateSignal` in `src/quant/ruleEngine.ts`)**: An extremely precise matrix evaluates potential positions based on the provided `techniquesList`. It now strictly enforces a minimum of 10 techniques for non-bypassed analysis to ensure strong consensus. The function accepts up to 5 arguments: `ohlcSeries`, `techniquesList`, `horizonArg`, `_confirmedPatterns`, and `_confirmedGaps`.
...
- **Output Verdict**: Computes a deterministic binary signal: `LONG` or `NO_TRADE` (mapped via the final rules logic). The verdict now tracks `shardPassVotes` and `techEnginePassVotes` to properly monitor technique engine votes over shards, capturing internal breakdown telemetry for the audit trail. Finally, the core analysis pipeline filters the resulting decision using `emitStability(decision)` to ensure robust final verdicts.

...
The initial pipeline execution natively begins within the isolated client frontend, specifically in `useBotLoop.ts` and UI hooks dealing with test mode or background analysis processing.
...
- **Pointwise Inequalities Matrix (`evaluateSignal` in `src/quant/ruleEngine.ts`)**: An extremely precise matrix evaluates potential positions.
...
- **Output Verdict**: Computes a deterministic binary signal: `LONG` or `NO_TRADE` (mapped via the final rules logic).

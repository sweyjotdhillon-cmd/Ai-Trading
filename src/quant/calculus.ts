...
export function firstDerivative(series: number[] | Float64Array, h = 1): Float64Array {
...
export function secondDerivative(series: number[] | Float64Array, h = 1): Float64Array {
  const n = series.length;
...
export function smoothBeforeDeriv(series: number[] | Float64Array, window = 3): Float64Array {
...
export function emaSlope(closes: number[] | Float64Array, period = 21): Float64Array {
  const emaValues = ema(closes, period);
  return firstDerivative(emaValues);
}

export function emaCurvature(closes: number[] | Float64Array, period = 21): Float64Array {
  const emaValues = ema(closes, period);

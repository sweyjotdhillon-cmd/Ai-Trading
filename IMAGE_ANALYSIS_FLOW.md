...
- **The Source:** The frontend, constructed using React & React Native Web (`twrnc`), intercepts user-provided images or real-time camera frames (during live mode, bulk test mode, or the new bot mode).
...
- **Payload Unpacking:** The worker receives the message, parsing out the raw pixels and user context (e.g., active techniques, horizon contexts, and scalping/risk configuration).
...
- **Scalping Feature Extraction:** The system derives `vwapProxy`, measures volatility with `atr`, and identifies key market structures via `findSwingPivots()`.
...
2. **Judge 2 (Oscillator Consensus):** Checks RSI divergence, MACD crossovers, and Stochastic extremes. Now fully integrates MACD as part of its dynamic intrinsic baseline.
3. **Judge 3 (Boundary & Reversal):** Inspects Bollinger Band pinches, Z-Score breakouts, and support/resistance interactions.
4. **Judge 4 (The Skeptic Multiplier):** An overriding penalty system that adjusts confidence scores if market conditions suggest untradeable noise (e.g., flat ATR-normalized slope) or verifies trends during momentum breakouts.

**The Verdict:** The four judges collapse their findings. Depending on the trading configuration (e.g. Bot Mode / Scalp Mode), the system calculates a `finalConfidence`, determines a robust entry signal (such as `LONG` or `NO_TRADE`), and logs details via (`JUDGE_LOG`). When scalping mode is enabled, `evaluateScalpSignal()` applies advanced risk state and pivoting logic before generating the final action.
...
- **Temporal Stability:** The final decision is passed through `emitStability()` (or `emitScalpStability()`) to prevent flickering UI signals. A signal only manifests if it has survived multiple sequential frames.
- **Message Transport:** The worker posts the resulting verdict, extracted patterns, scalping decisions, and real-time calculation logs back to the main thread.
- **UI Render:** `LiveAnalysis.tsx` consumes the event payload. The application instantly maps the highly precise signal logic to the visual components (`LiveAnalysisResult`, `LiveAnalysisDebate`), displaying clear, high-contrast insights like "EXECUTE LONG" or "HOLD".
